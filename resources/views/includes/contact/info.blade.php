@if($defaultLocation)
    <div class="bg-body dark:bg-surface rounded-lg shadow-lg p-6">
        ...
        <div class="space-y-4">
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-text-muted dark:text-text-muted">
                            {!! format_address($defaultLocation->getAddress()) !!}
                        </p>
                    </div>
                </div>
            @endif
            ...
            <!-- Phone -->
            @if($defaultLocation->getTelephone())
                <div class="flex items-start gap-3">
                    <div class="shrink-0 w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center">
                        <svg class="w-5 h-5 text-primary-600 dark:text-primary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex-1">
                        ...
                        <a
                            href="tel:{{ $defaultLocation->getTelephone() }}"
                            class="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300"
                        >
                            {{ $defaultLocation->getTelephone() }}
                        </a>
                    </div>
                </div>
            @endif
